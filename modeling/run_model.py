...
def run(df: pd.DataFrame,
    ...
    train, test, y_train, y_test = train_test_split(df.drop(columns=['target']), df.target.astype(int),
                                                    test_size=0.33)
    ...
    with mlflow.start_run():
        ...
        importance_df = pd.DataFrame(
            {'feature': clf.feature_names_, 'importance': clf.get_feature_importance(
        ...
        for features_num in tqdm(features_num_interval):
            ...
            selector = SelectFromModel(estimator=clf,
                                       max_features=features_num).fit(train,
                                                                      y_train,
                                                                      cat_features=cat_features,
                                                                      plot=False,
                                                                      verbose=False)  ## todo: добавить eval_data
            ...
            cat_model = CatBoostClassifier(**params, cat_features=new_cat_features,
                                           eval_metric='AUC:hints=skip_train~false',
                                           verbose=False,
                                           random_state=13)
